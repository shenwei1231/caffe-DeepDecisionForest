...
parser = argparse.ArgumentParser(description='Deep Differentiable Random Forests for Age Estimation')
parser.add_argument('--method', type=str, required=False, choices=['DRF', 'DLDLF'], default='DRF')
...
parser.add_argument('--T', type=float, required=False, default=0.0001)
...
def make_net(phase='train'):
  ...
  n.fc8 = L.InnerProduct(n.drop7, num_output=nout, bias_term=True, weight_filler=dict(type='gaussian', std=0.005), bias_filler=dict(type='constant', value=0),
            param=[dict(lr_mult=10, decay_mult=1), dict(lr_mult=20, decay_mult=0)], name='fc8-101')#name='fc8a')
  ...
  if phase=='train':
    ...
    if args.method == 'DRF':
      n.loss = L.NeuralDecisionRegForestWithLoss(n.fc8, n.label, param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0),dict(lr_mult=0, decay_mult=0)], 
          neural_decision_forest_param=dict(depth=treeDepth, num_trees=ntree, num_classes=1, iter_times_class_label_distr=20, beta=args.beta, tt=args.T,
              iter_times_in_epoch=50, all_data_vec_length=all_data_vec_length, drop_out=args.drop, init_filename=args.init,record_filename='F_morph.record'), name='probloss')
    elif args.method == 'DLDLF':
      n.loss = L.NeuralDecisionDLForestWithLoss(n.fc8, n.label, param=[dict(lr_mult=0, decay_mult=0), dict(lr_mult=0, decay_mult=0)], 
          neural_decision_forest_param=dict(depth=treeDepth, num_trees=ntree, num_classes=maxAge-minAge+1, iter_times_class_label_distr=20, tt=args.T,
              iter_times_in_epoch=50, all_data_vec_length=all_data_vec_length, drop_out=args.drop, init_filename=args.init,record_filename='F_morph.record'), name='probloss')
  elif phase=='test':
    if args.method == 'DRF':
      n.pred = L.NeuralDecisionRegForest(n.fc8, n.label, neural_decision_forest_param=dict(depth=treeDepth, num_trees=ntree, num_classes=1), name='probloss')
    elif args.method == 'DLDLF':
    ...
    n.CS5 = L.CS(n.pred, n.label,cs_param = dict(cs_l=5))
  elif phase=='deploy':
    n.pred = L.NeuralDecisionRegForest(n.fc8, neural_decision_forest_param=dict(depth=treeDepth, num_trees=ntree, num_classes=1), name='probloss')
  return n.to_proto()
