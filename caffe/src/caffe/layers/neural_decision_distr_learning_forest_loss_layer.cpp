...
void RandSample(int num_samples, int num_sub_samples, Dtype* sample_index_vec) {
    vector<int> nind(num_samples, 0);
    for (int i = 0; i < num_samples; ++i) nind[i] = i;
...
template <typename Dtype>
void NeuralDecisionDLForestWithLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    debug_gpu_ = neural_decision_forest_param.debug_gpu();
    use_gpu_ = neural_decision_forest_param.use_gpu();
    ...
    num_epoch_ = 0;
    if (neural_decision_forest_param.record_filename().length() > 0)
        of_.open(neural_decision_forest_param.record_filename().c_str());
    T_ = neural_decision_forest_param.tt();
}

template <typename Dtype>
void NeuralDecisionDLForestWithLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    ...
    routing_leaf_all_data_prob_vec_[iter_times_ % all_data_vec_length_].reset(
        new Blob<Dtype>(num_outer_, num_inner_, num_trees_, num_leaf_nodes_per_tree_));
    tree_prediction_all_data_prob_vec_[iter_times_ % all_data_vec_length_].reset(
        new Blob<Dtype>(num_outer_, num_inner_, num_trees_, num_classes_));
    all_data_label_vec_[iter_times_ % all_data_vec_length_].reset(
        new Blob<Dtype> (num_outer_, num_inner_, num_classes_, 1));
...
template <typename Dtype>
void NeuralDecisionDLForestWithLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    ...
    Dtype* routing_leaf_all_data_prob_data = 
    routing_leaf_all_data_prob_vec_[iter_times_ % all_data_vec_length_].get()->mutable_cpu_data();
    ...
    for (int i = 0; i < num_outer_; i++) {
        for (int k = 0; k < num_inner_; k++) {
            ...
            if (drop_out_) {
                ...
                //forest_prediction_prob += tree_prediction_prob_.data_at(i, t, static_cast<int>(label[i]), 0);
            } else {
                for(int t = 0; t < num_trees_; t++){
                    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, 1, num_classes_, num_leaf_nodes_per_tree_,
...
template <typename Dtype>
void NeuralDecisionDLForestWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    if (iter_times_+1 >= all_data_vec_length_ && (iter_times_+1)%iter_times_in_epoch_ == 0) UpdateClassLabelDistr();
    iter_times_++;
}
