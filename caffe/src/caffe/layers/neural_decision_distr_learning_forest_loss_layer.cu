...
__global__ void kernel_backward_dl(Dtype* bottom_diff, Dtype* inter_data, const Dtype* cls_lb_distr, const Dtype* label_data, 
                                   ...
                                   int h, int w, int num_classes, int tree_id, int num_dims_, Dtype* inter_prob, Dtype T_) {
  ...
  CUDA_KERNEL_LOOP(index, num_outer) {
    for (int i=0; i<num_inner; ++i) {
      for (int l=0; l<num_leaf; ++l) {
        ...
        int prob_idx = sub2ind_dl(index,i,tree_id,num_split+l, num_outer, num_inner, num_trees,num_nodes);
        inter_prob[prob_idx] = routing_lf[routing_lf_idx] * (1 + log(max(routing_lf[routing_lf_idx], Dtype(FLT_MIN))));
      }
      for (int n=num_split-1; n>=0; --n) {
        ...
        int prob_parent_idx = sub2ind_dl(index,i,tree_id,n,num_outer,num_inner,num_trees,num_nodes);
        int prob_left_idx = sub2ind_dl(index,i,tree_id,2*n+1,num_outer,num_inner,num_trees,num_nodes);
        int prob_right_idx = prob_left_idx + 1;
        bottom_diff[diff_idx] += T_ * (- dn_data[diff_idx] * inter_prob[prob_right_idx]
                  + (Dtype(1.0) - dn_data[diff_idx]) * inter_prob[prob_left_idx]);
        inter_prob[prob_parent_idx] = inter_prob[prob_left_idx] + inter_prob[prob_right_idx];
      }
    }
  }
}
...
__global__ void kernel_backward_all_dl(Dtype* bottom_diff, Dtype* inter_data, const Dtype* cls_lb_distr, const Dtype* label_data, 
                                   const Dtype* routing_lf, const Dtype* dn_data, const Dtype* tree_pred, const Dtype* dim_offset,
                                   int num_outer, int num_inner, int num_trees, int num_leaf, int num_split, 
                                   int h, int w, int num_classes, int num_dims_, Dtype* inter_prob, Dtype T_) {
  int num_nodes = num_split + num_leaf;
  CUDA_KERNEL_LOOP(index, num_outer) {
    for (int i=0; i<num_inner; ++i) {
      for (int l=0; l<num_leaf; ++l) {
        for(int t= 0; t < num_trees; t++) {
          ...
          int prob_idx = sub2ind_dl(index,i,t,num_split+l, num_outer, num_inner, num_trees,num_nodes);
          inter_prob[prob_idx] = routing_lf[routing_lf_idx] * (1 + log(max(routing_lf[routing_lf_idx], Dtype(FLT_MIN))));
        }
      }
      for (int n=num_split-1; n>=0; --n) {
        for(int t = 0; t < num_trees; t++) {
          ...
          int prob_parent_idx = sub2ind_dl(index,i,t,n,num_outer,num_inner,num_trees,num_nodes);
          int prob_left_idx = sub2ind_dl(index,i,t,2*n+1,num_outer,num_inner,num_trees,num_nodes);
          int prob_right_idx = prob_left_idx + 1;
          bottom_diff[diff_idx] += T_ * (- dn_data[diff_idx] * inter_prob[prob_right_idx]
                    + (Dtype(1.0) - dn_data[diff_idx]) * inter_prob[prob_left_idx]);
          inter_prob[prob_parent_idx] = inter_prob[prob_left_idx] + inter_prob[prob_right_idx];
...
void NeuralDecisionDLForestWithLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, 
  ...
  if (!use_gpu_)
    ...
    if (propagate_down[0])  {
      ...
      Blob<Dtype> inter_prob(num_outer_, num_inner_, num_trees_, num_leaf_nodes_per_tree_ + num_split_nodes_per_tree_);
      ...
      if (drop_out_) {
        ...
        dn_->width(), num_classes_, tree_for_training_, num_dims_, inter_prob.mutable_gpu_data(), T_);
      } else {
        kernel_backward_all_dl<Dtype><<<CAFFE_GET_BLOCKS(num_outer_), CAFFE_CUDA_NUM_THREADS>>>(
        bottom[0]->mutable_gpu_diff(), inter_var_.mutable_gpu_data(), class_label_distr_->gpu_data(),
        bottom[1]->gpu_data(), routing_leaf_prob_.gpu_data(), dn_->gpu_data(),tree_prediction_prob_.gpu_data(), sub_dimensions_->gpu_data(),
        num_outer_, num_inner_,num_trees_, num_leaf_nodes_per_tree_, num_split_nodes_per_tree_, dn_->height(),
        dn_->width(), num_classes_, num_dims_, inter_prob.mutable_gpu_data(), T_);
      }
      if (debug_gpu_) {
      ...
      // Scale down gradient
      int count = num_outer_ * num_inner_;
      if (!drop_out_){
          count *= num_trees_;
      }
      ...
      //caffe_scal(bottom[0]->count(), loss_weight / get_normalizer(normalization_, count), bottom[0]->mutable_cpu_diff());  bug
